...
def load_wiki_context() -> str:
    wiki_path = Path(__file__).parent / "wiki_context.md"
    if wiki_path.exists():
...
# Load wiki knowledge base
_wiki_context = load_wiki_context()
_wiki_section = (
    f"\n\nKNOWLEDGE BASE (from my personal wiki — things I've studied and learned):\n\n{_wiki_context}"
    if _wiki_context else ""
)
...
my_agent_twin = Agent(
    ...
    - I'm interested in human and technology interface
    - I grew up in China, spent 10 years in the US doing public policy grad school and working in technology; Now I'm based in the Netherlands, working for a non-profit called B Lab
    - I'm curious about systemic change, both in the economic system and broader living world
    - I want to help people ask better questions, and make better decisions that fits their particular circumstances
    - I value intellectual humility, saying I don't know when I don't
    - I like climbing outdoor, but still quite scared about falling - especially on the slab
    - I am learning French, trying to sing some chanson francais
    - I love experimenting with new tools like CrewAI and exploring how we can make ourselves better humans with AI
    - I can read R, SQL, Python but I'm not great at writing code from scratch
...
async def fetch_agents_from_registry():
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            for agent in agents:
                # username may be top-level or nested inside agentFacts
                username = (agent.get("agent_id")
                            or agent.get("agentFacts", {}).get("username")
...
"""
    ...
    try:
        ...
        selection_llm = LLM(model="anthropic/claude-haiku-4-5-20251001", temperature=0.3)
...
async def query_agent(request: QueryRequest):
    ...
    try:
        ...
        # Create crew with memory enabled (ONNX embedder avoids OpenAI dependency)
        crew = Crew(
